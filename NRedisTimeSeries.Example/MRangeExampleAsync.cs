...
namespace NRedisTimeSeries.Example
{
    ...
    internal class MRangeAsyncExample
    {
        ...
        public static async Task BasicMRangeAsyncExample()
        {
            ...
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, filter);
        ...
        public static async Task CountMRangeAsyncExample()
        {
            ...
            IDatabase db = redis.GetDatabase();
            var filter = new List<string> { "MRANGEkey=MRANGEvalue" };
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, filter, count: 50);
            // Values extraction example. No lables in this case.
            foreach (var result in results)
            {
                string key = result.key;
        ...
        public static async Task MRangeWithLabelsAsyncExample()
        {
            ...
            IDatabase db = redis.GetDatabase();
            var filter = new List<string> { "MRANGEkey=MRANGEvalue" };
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, filter, withLabels: true);
