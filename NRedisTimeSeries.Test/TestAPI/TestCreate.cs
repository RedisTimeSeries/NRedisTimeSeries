...
namespace NRedisTimeSeries.Test.TestAPI
{
    public class TestCreate : AbstractTimeSeriesTest, IDisposable
    {
        ...
        {
            IDatabase db = redisFixture.Redis.GetDatabase();
            Assert.True(db.TimeSeriesCreate(key, duplicatePolicy: TsDuplicatePolicy.FIRST));
        }
        ...
        {
            IDatabase db = redisFixture.Redis.GetDatabase();
            Assert.True(db.TimeSeriesCreate(key, duplicatePolicy: TsDuplicatePolicy.LAST));
        }
        ...
        {
            IDatabase db = redisFixture.Redis.GetDatabase();
            Assert.True(db.TimeSeriesCreate(key, duplicatePolicy: TsDuplicatePolicy.MIN));
        }
        ...
        {
            IDatabase db = redisFixture.Redis.GetDatabase();
            Assert.True(db.TimeSeriesCreate(key, duplicatePolicy: TsDuplicatePolicy.MAX));
        }
        ...
        {
            IDatabase db = redisFixture.Redis.GetDatabase();
            Assert.True(db.TimeSeriesCreate(key, duplicatePolicy: TsDuplicatePolicy.SUM));
        }
    }
}
