...
namespace NRedisTimeSeries.Test.TestAPI
{
    public class TestMRangeAsync : AbstractTimeSeriesTest
    {
        ...
        public async Task TestSimpleMRange()
        {
            ...
            var results = db.TimeSeriesMRange(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string> { $"{keys[0]}=value" });
        ...
        public async Task TestMRangeWithLabels()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string> { $"{keys[0]}=value" }, withLabels: true);
            Assert.Equal(keys.Length, results.Count);
            for (var i = 0; i < results.Count; i++)
            {
                Assert.Equal(keys[i], results[i].key);
        ...
        public async Task TestMRangeFilter()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string> { $"{keys[0]}=value" });
        ...
        public async Task TestMRangeCount()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var count = 5L;
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string> { $"{keys[0]}=value" }, count: count);
            Assert.Equal(keys.Length, results.Count);
            for (var i = 0; i < results.Count; i++)
            {
                Assert.Equal(keys[i], results[i].key);
        ...
        public async Task TestMRangeAggregation()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var results = await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string> { $"{keys[0]}=value" }, aggregation: Aggregation.MIN, timeBucket: new TsTimeBucket(50));
            Assert.Equal(keys.Length, results.Count);
            for (var i = 0; i < results.Count; i++)
            {
                Assert.Equal(keys[i], results[i].key);
        ...
        public async Task TestMissingFilter()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var ex = await Assert.ThrowsAsync<ArgumentException>(async () => await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue, new List<string>()));
        ...
        public async Task TestMissingTimeBucket()
        {
            ...
            {
                await db.TimeSeriesMRangeAsync(TsTimeStamp.MinValue, TsTimeStamp.MaxValue,
