...
namespace NRedisTimeSeries.Test.TestAPI
{
    public class TestMRangeAsync : AbstractTimeSeriesTest
    {
        ...
        public async Task TestMRangeGroupby()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var results = await db.TimeSeriesMRangeAsync("-", "+", new List<string> { $"{keys[0]}=value" }, withLabels: true, groupby: "group", reduce: TsReduce.Min);
            Assert.Equal(keys.Length, results.Count);
        ...
        public async Task TestMRangeReduce()
        {
            ...
            var tuples = await CreateData(db, keys, 50);
            var results = await db.TimeSeriesMRangeAsync("-", "+", new List<string> { $"{keys[0]}=value" }, withLabels: true, groupby: keys[0], reduce: TsReduce.Sum);
            Assert.Equal(1, results.Count);
