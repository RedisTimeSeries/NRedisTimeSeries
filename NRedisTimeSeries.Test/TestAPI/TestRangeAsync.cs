...
namespace NRedisTimeSeries.Test.TestAPI
{
    public class TestRangeAsync : AbstractTimeSeriesTest
    {
        ...
        public async Task TestRangeAggregation()
        {
            ...
            var db = redisFixture.Redis.GetDatabase();
            var tuples = await CreateData(db, key, 50);
            Assert.Equal(tuples, await db.TimeSeriesRangeAsync(key, TsTimeStamp.MinValue, TsTimeStamp.MaxValue, aggregation: Aggregation.MIN, timeBucket: new TsTimeBucket(50)));
        }
